...
import { CATEGORY_COLORS, CATEGORY_ICONS } from '../../../src/utils/helpers';
...
export default function EditCategoryScreen() {
  ...
  const handleSave = () => {
    if (!name.trim()) { Alert.alert('Nom requis', 'Veuillez entrer un nom.'); return; }
    updateCategory({ id, name: name.trim(), color, icon });
    router.back();
  };
  ...
  return (
    <KeyboardAvoidingView style={[styles.screen, { backgroundColor: t.bg }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.header, { paddingTop: insets.top + 8, backgroundColor: t.surface, borderBottomColor: t.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: t.text }]}>Modifier la catégorie</Text>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <TextInput value={name} onChangeText={setName} placeholderTextColor={t.textTertiary}
...
const styles = StyleSheet.create({
  ...
  saveBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20 },
